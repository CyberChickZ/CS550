...
const char *WINDOWTITLE = "OpenGL / EECS 550 p2 -- CyberChickZ";
...
// my var
GLuint  WireHorseList = 0;
GLuint  SolidHorseList = 0;
...
InitLists( )
{
	...
	// === Circle path in X-Z plane, radius PATH_RADIUS ===
	CircleList = glGenLists(1);
	glNewList(CircleList, GL_COMPILE);
		glColor3f(0.f, 1.f, 1.f);           // cyan
		const int SEGMENTS = 256;           // smooth circle
		glBegin(GL_LINE_LOOP);
			for (int i = 0; i < SEGMENTS; ++i) {
				float t = (float)i / (float)SEGMENTS;
				float ang = t * 2.f * F_PI; // radians
				float x = PATH_RADIUS * cosf(ang);
				float z = PATH_RADIUS * sinf(ang);
				glVertex3f(x, 0.f, z);      // y = 0 in X-Z plane
			}
		glEnd();
	glEndList();
